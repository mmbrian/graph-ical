...
import { NODE_LABEL_PADDING, toggleCluster } from "./index";
const hx = require("hexagon-js");

export const NODE_CONTROLS_STROKE_WEIGHT = 32;
export const ZOOM_EXTENTS = [1, 8];
...
export const renderAddEdgeIndicatorOnNodeHover = (
    ...
    let nodeAddEdgeIndicatorStream = combineLatest(
        ...
        map(args => {
            if (args[4].finished && args[0].exit && args[1].enter && !args[1].node.isClustedNode) {
                return {
                    render: true,
                    node: args[1].node,
                    mouseXY: args[2].m
...
export const renderNodeControlsOnHover = (
    g, nodeEnterExitStream, globalSvgMouseMoveStream,
    ...
    clusterToggle.on("click", () => {
        ...
        toggleCluster(targetNode.node_type);
    });
    clusterToggle
        .on("mouseover", () => {
            d3.select(".cursor-content").classed("tooltip", true);
            ...
            if (payload.clicked_node) {
                ...
                if (payload.isDisplayingControls) {
                    ...
                    indicatorCircleOuter
                        ...
                        .attr("cy", payload.updatedNode.y)
                        .classed("hidden", false)
                        .attr("r", payload.updatedNode.innerRadius + NODE_CONTROLS_STROKE_WEIGHT);
                    renameToggle
                        .classed("hidden", false)
                        .attr("x", payload.updatedNode.x + payload.updatedNode.innerRadius + NODE_CONTROLS_STROKE_WEIGHT/2)
                    ...
                    if (payload.updatedNode.isClustedNode) {
                        clusterToggle
                            ...
                            .attr("y", payload.updatedNode.y - payload.updatedNode.innerRadius - NODE_CONTROLS_STROKE_WEIGHT/2);
